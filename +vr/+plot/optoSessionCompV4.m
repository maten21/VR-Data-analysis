...
% V4 I 2022.01.05 I made this version because averaging sessions was not working  
%
%
%
%
...
for t = 1:1:14
    ...
    if ~isempty(allData(t).OptProt)
        
        g = 1;
        h = 1;
        
        for f = 1:1:numel(allData(t).mouseNames) %calculate average matrices
             
            if f == numel(allData(t).mouseNames) || ~strcmp(allData(t).mouseNames{f+1},allData(t).mouseNames{g})
                
                % initiate empty arrays
                distr50 = zeros(size(allData(t).lickDistr50(1,:)));
                firstL50 = zeros(size(allData(t).lickDistr50(1,:)));
                hits = zeros(size(allData(t).lickDistr50(1,:)));
                freqs = zeros(size(allData(t).lickDistr50(1,:)));
                vels = zeros(size(allData(t).lickDistr50(1,:)));
                times = zeros(size(allData(t).lickDistr50(1,:)));
                
                for i = 1:1:f-g+1
                    
                    distr50 = distr50 + allData(t).lickDistr50(g+i-1,:);
                    firstL50 = firstL50 + allData(t).firstLicks50(g+i-1,:);
                    
                    hits = hits + allData(t).hitRate(g+i-1,:);
                    freqs = freqs + allData(t).trialFreqs(g+i-1,:);
                    vels = vels + allData(t).trialVels(g+i-1,:);
                    times = times + allData(t).trialTimes(g+i-1,:);
                end
                
                
                mouseAvs(t).lickDistr50(h,:) = distr50/i;
                mouseAvs(t).firstLicks50(h,:) = firstL50/i;
                
                mouseAvs(t).hitRate(h,:) = hits/i;
                mouseAvs(t).trialFreqs(h,:) = freqs/i;
                mouseAvs(t).trialVels(h,:) = vels/i;
                mouseAvs(t).trialTimes(h,:) = times/i;
...
%%
% allData = allData60; Xax = [3 4];
% allData = allData10; Xax = [1 2];




%% Plot SUMMARY FIG INDIVIDUAL SESSIONS or MOUSE AVERAGES

% PICK THE RELEVANT:

% fileName = 'opsinFirstSessionsSummary4mice'; data = allData;
% fileName = 'opsinAllDataSummary4mice'; data = allData;
% fileName = 'controlAllDataSummary'; data = allData;
% fileName = 'maskStimAllDataSummary'; data = allData;
% fileName = 'controlFirstSessionsSummary10mice'; data = allData;

% fileName = 'opsinMouseAveragesSummarymice'; data = mouseAvs;
% fileName = 'cotrolMouseAveragesSummary'; data = mouseAvs;
...
for i =1:1:numel(trialTypeSubset)
    ...
    y =  data(trialTypeSubset(i)).lickDistr50(:,:);
...
for i =1:1:numel(trialTypeSubset)
    ...
    y =  data(trialTypeSubset(i)).firstLicks50(:,:);
...
for i =1:1:numel(trialTypeSubset)
    ...
    y =  data(trialTypeSubset(i)).hitRate(:,:);
...
for i =1:1:numel(trialTypeSubset)
    ...
    y =  data(trialTypeSubset(i)).trialVels(:,:);
...
for i =1:1:numel(trialTypeSubset)
    ...
    y =  data(trialTypeSubset(i)).trialFreqs(:,:);
...
for i =1:1:numel(trialTypeSubset)
    ...
    y =  data(trialTypeSubset(i)).trialTimes(:,:);
...
% fileName = 'opsinallDataSummary4mice'; 
% fileName = 'controlallDataSummary'; 
% fileName = 'opsinMouseAveragesSummary4mice'; 
% fileName = 'controlMouseAveragesSummary'; 
...
%% Plot fig MOUSE AVERAGES (exactly the same as the previous figure only the allData is replaced by mouseAvs
%{
% Xcount = nTrialTypes; Xcount = 4;
xticks = 0:1:(numel(trialTypeSubset)*2 + 1);
...
for i =1:1:numel(trialTypeSubset)
    ...
    y =  mouseAvs(trialTypeSubset(i)).lickDistr50(:,:);
...
for i =1:1:numel(trialTypeSubset)
    ...
    y =  mouseAvs(trialTypeSubset(i)).firstLicks50(:,:);
...
for i =1:1:numel(trialTypeSubset)
    ...
    y =  mouseAvs(trialTypeSubset(i)).hitRate(:,:);
...
for i =1:1:numel(trialTypeSubset)
    ...
    y =  mouseAvs(trialTypeSubset(i)).trialVels(:,:);
...
for i =1:1:numel(trialTypeSubset)
    ...
    y =  mouseAvs(trialTypeSubset(i)).trialFreqs(:,:);
...
for i =1:1:numel(trialTypeSubset)
    ...
    y =  mouseAvs(trialTypeSubset(i)).trialTimes(:,:);
...
% data1 = allData.trialTimes./allData.hitRate;
...
% data1 = allData.firstLicks50; Xlabel = 'First lick in control (cm, median)';
% data1 = allData.lickDistr50; Xlabel = 'Lick distribution in control (cm, median)';
% data1 = allData.hitRate; Xlabel = 'Hitrate in control (%)';
% data1 = allData.trialVels; Xlabel = 'Velocity in control (cm/s)';
% data1 = allData.trialFreqs; Xlabel = 'Lick rate in control (Hz)';
% data1 = allData.trialTimes; Xlabel = 'Trial time in control (s)';

% data1 = allData.trialVels; Xlabel = 'Change in velocity (cm/s)';
% data1 = allData.trialFreqs; Xlabel = 'Change in lick rate (Hz)';

% data2 = allData.firstLicks50; Ylabel = 'Shift of first lick';
% data2 = allData.lickDistr50; Ylabel = 'Shift of lick distr.';
% data2 = allData.hitRate; Ylabel = 'Change in hit rate';
% data2 = allData.trialVels; Ylabel = 'Change in velocity (cm/s)';
% data2 = allData.trialFreqs; Ylabel = 'Change in lick rate (Hz)';
% data2 = allData.trialTimes; Ylabel = 'Change in trial duration (s)';
...
rewardTimes =  allData.trialTimes./(allData.hitRate/100);
