...
for sortMat = 1:2
    ...
    data = smoothdata(normalize(permute(sData.imdata(fov).binnedRoisDff,[3 2 1]),2),2,'gaussian',smoothSpan);
...
plot((-shift:shift)*2,nanmean(A1A2s(bins,:)))
plot((-shift:shift)*2,nanmean(B1B2s(bins,:)))
plot((-shift:shift)*2,nanmean(ABs(bins,:)))
...
mymap = lines;
bins = [1:24 96:125];
plotMeanStd((-shift:shift)*binSize, [Cs2(bins,:)', Cs3(bins,:)', Cs5(bins,:)', Cs6(bins,:)'],mymap(1,:),1)
bins = 25:95;
plotMeanStd((-shift:shift)*binSize, [Cs2(bins,:)', Cs3(bins,:)', Cs5(bins,:)', Cs6(bins,:)'],mymap(2,:),1)
